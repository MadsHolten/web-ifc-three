    ...
    private getGeometryAndMaterialsIndividual() {
        ...
        for (let materialID in items) {
            ...
            if (items.hasOwnProperty(materialID)) {
                ...
                Object.keys(items[materialID].geometries).forEach((expressID: string) => {

                    const geometry: any = items[materialID].geometries[expressID];
                    const geometryMaterial: GeometryMaterial = { geometry, material };
