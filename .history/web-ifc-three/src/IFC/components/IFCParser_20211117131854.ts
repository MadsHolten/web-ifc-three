...
export class IFCParser implements ParserAPI {
    ...
    private async loadAllGeometry() {
        ...
        await this.buildMHRAObject(); // Build the object
    ...
    private buildMHRAObject(){
        ...
        const {geometry, materials} = this.getGeometryAndMaterialsIndividual();
    ...
    private getGeometryAndMaterialsIndividual() {
        ...
        const mergedByMaterial: BufferGeometry[] = []; // UD
        const materials: Material[] = []; // UD

        // Loop over hovedmodellens items
        for (let materialID in items) {
            ...
            if (items.hasOwnProperty(materialID)) {
                ...
                console.log(items[materialID].material);
